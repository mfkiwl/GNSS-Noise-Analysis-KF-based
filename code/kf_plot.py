# -*- coding: utf-8 -*-
"""
Created on Fri Dec 20 16:12:08 2024
...
def plot_kf_ts(time, gps_y, gps_y_sig, gps_full_decy, 
    ...
    ax0.plot(gps_full_decy, wls_fit, c="r", lw=2, zorder=3, label="KF")
    ...
    plt.tight_layout() 
    plt.savefig(output_path + name + "/kf_ts.png", dpi=300)
    plt.close("all")


def plot_kf_res(gps_full_decy, wls_res, freq_cpy, psd_lom, name, output_path, component_str):
    ...
    ax0.set_title(name + " (YZ-fixed) " + component_str + " KF Residual", fontsize=34, pad=10)
    ...
    plt.tight_layout() 
    plt.savefig(output_path + name + "/kf_res.png", dpi=300)
    plt.close("all")

    
def plot_kf_comp(gps_full_decy, offset_decy, wls_trend, wls_annual, wls_semi_annual, 
                  wls_color, wls_res, name, output_path, component_str):
    ...
    ax3.plot(gps_full_decy, wls_color, c="k", linewidth=0.5)
    ax3.plot(gps_full_decy, wls_color, 'o', c="k", ms=2.8)
    ax3.yaxis.set_minor_locator(AutoMinorLocator(5))
    
    ax4.plot(gps_full_decy, wls_res, c="k", linewidth=0.5)
    ax4.plot(gps_full_decy, wls_res, 'o', c="k", ms=3.5)
    ...
    ax0.set_title(name+" (YZ-fixed) "+component_str+" KF",fontsize=32,pad=15)
    ...
    ax3.set_ylabel("color noise (mm)",fontsize=22,labelpad=8)
    ax4.set_ylabel("residual (mm)",fontsize=22,labelpad=8)
    ax4.set_xlabel("time in year",fontsize=22,labelpad=8)
    
    plt.tight_layout(); 
    plt.savefig(output_path + name + "/kf_comp.png", dpi=300)
    
    plt.close("all")
    
    
def plot_kf_ts_unc(time, gps_y, gps_y_sig, gps_full_decy, 
                wls_fit, wls_unc, offset_decy, name, component_str, output_path): 
    ...
    ax0.plot(time, gps_y, "o", c="k", ms=2.5, zorder=2, label="raw data")
    ax0.plot(gps_full_decy, wls_fit, c="r", lw=2, zorder=4, label="KF")
    
    ax0.fill_between(gps_full_decy, wls_fit - 2 * wls_unc, 
                     wls_fit + 2 * wls_unc, color='r', alpha=0.24, zorder=3, label="2$\sigma$ error")
    ax0.plot(gps_full_decy, wls_fit - 2 * wls_unc, c="r", ls='-.' ,lw=1, zorder=4)
    ax0.plot(gps_full_decy, wls_fit + 2 * wls_unc, c="r", ls='-.' ,lw=1, zorder=4)
    ...
    plt.tight_layout() 
    plt.savefig(output_path + name + "/kf_ts_unc.png", dpi=300)
    plt.close("all")