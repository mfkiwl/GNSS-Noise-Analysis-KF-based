# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 09:45:30 2024
...
# local
from wls_plot import plot_wls_ts, plot_wls_res, plot_wls_comp
from kf import kf_main, rts, mle_apso
from kf_plot import plot_kf_ts, plot_kf_res, plot_kf_comp, plot_kf_ts_unc
...
for st in gps_list:
    ...
    _, wls_result = wls(time, offset_decy, post_const, post_decy, yw, gps_y_sig)
    ...
    #=================================================================
    # Plot
    plot_wls_ts(time, gps_y, gps_y_sig, gps_full_decy,
                wls_fit, offset_decy, name, component_str, OUTPUT_DIR)
    
    plot_wls_res(gps_full_decy, wls_res, freq_cpy, psd_lom, name,
                 OUTPUT_DIR, component_str)
    
    plot_wls_comp(gps_full_decy, offset_decy, wls_trend, wls_annual, wls_semi_annual,
                  wls_res, name, OUTPUT_DIR, component_str)
    ...
    #==============================================
    # Plot
    plot_kf_ts(time, gps_y, gps_y_sig, gps_full_decy,
                rts_y, offset_decy, name, component_str, OUTPUT_DIR)
    
    plot_kf_res(gps_full_decy, kf_res, freq_cpy, psd_lom_kf, name,
                 OUTPUT_DIR, component_str)
    
    plot_kf_comp(gps_full_decy, offset_decy, kf_trend, kf_annual, kf_semi_annual,
                  kf_color, kf_res, name, OUTPUT_DIR, component_str)
    
    plot_kf_ts_unc(time, gps_y, gps_y_sig, gps_full_decy,
                rts_y, rts_y_var, offset_decy, name, component_str, OUTPUT_DIR)
    break
    #==============================================
    # output
    with open(OUTPUT_DIR + name + "/pso.txt", 'w') as f:
        f.write("# -logL : " + str(pso_score[-1]) + '\n')
        f.write("# vel    ann    semi_ann    color    white\n")
        [f.write(str(a) + "    ") for a in pso_theta]
        
    with open(OUTPUT_DIR + name + "/rts_mu.txt", 'w') as f:
        for i in range(time_dim):
            f.write(str(gps_full_decy[i]) + "    ")
            [f.write(str(a) + "    ") for a in mu[i, :, 0]]
            f.write("\n")
            
    with open(OUTPUT_DIR + name + "/rts_sig.txt", 'w') as f:
        for i in range(time_dim):
            f.write(str(gps_full_decy[i]) + "    ")
            [f.write(str(a) + "    ") for a in rts_sig_diag[i]]
            f.write("\n")
            
    with open(OUTPUT_DIR + name + "/H.txt", 'w') as f:
        for i in range(time_dim):
            f.write(str(gps_full_decy[i]) + "    ")
            [f.write(str(a) + "    ") for a in H[i, 0, :]]
            f.write("\n")
            
    with open(OUTPUT_DIR + name + "/noise.txt", 'w') as f:
        f.write("# white_amp(R mean-sqrt) : " + str(white_amp) + "\n")
        f.write("# time    color    white\n")
        for i in range(time_dim):
            f.write(str(gps_full_decy[i]) + "    " +
                    str(kf_color[i])+ "    " +
                    str(kf_res[i])+ "\n")
         
    del cov, cov0, dt, F, f, freq, freq_cpy, freq_omega_cpy, gps_full_decy, gps_full_y,\
        ...
        pso_score, pso_theta, Q, R, rts_sig_diag, rts_y, rts_y_var, state_dim, \
        time_dim, white_amp
